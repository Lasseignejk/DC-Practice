...
# # In this exercise, are you using dynamic keys or fixed keys?
word = input("Please enter a word with at least three letters: ")
...
def letter_histogram():
    ...
    for letter in word:
        if letter not in dictionaryKeys:
            ...
            letterDictionary[letter] += 1
# The return keyword prints the result to the terminal, but also allows it to be stored as the result of a function. If you don't use return and just use print instead, the result will be printed but that's it. You can't use it for something else later.
...
def histogram_tally():
    ...
    while len(word) >= 3:
        ...
        tally = sorted(toSort.items(), key=get_value, reverse=True)
        print(
